...
public class GlideHelper {
    ...
    public static void loadUrl(Fragment mFragment, String url, ImageView imageView) {
        if (mFragment != null) {
    ...
    public static void loadFile(Fragment mFragment, File file, ImageView imageView) {
        if (mFragment != null) {
            GlideApp.with(mFragment)
                    .load(file)
                    .placeholder(R.drawable.img_glide_load_ing)
    ...
    public static void loadUrlToBitmap(Context mContext, String url, final OnGlideBitmapResultListener listener) {
        if (mContext != null) {
            GlideApp.with(mContext).asBitmap().load(url).centerCrop()
                    .skipMemoryCache(true)
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
                    .format(DecodeFormat.PREFER_RGB_565)
                    // 取消动画，防止第一次加载不出来
    ...
    public static void loadUrlToBitmap(Fragment mFragment, String url, final OnGlideBitmapResultListener listener) {
        if (mFragment != null) {
            GlideApp.with(mFragment).asBitmap().load(url).centerCrop()
                    .skipMemoryCache(true)
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
                    .format(DecodeFormat.PREFER_RGB_565)
                    // 取消动画，防止第一次加载不出来
